...
void dComIfG_playerStatusD() {
    ...
    dComIfGs_offItemFirstBit(L2_KEY_PIECES1);
    dComIfGs_offItemFirstBit(L2_KEY_PIECES2);
    dComIfGs_offItemFirstBit(L2_KEY_PIECES3);
    dComIfGs_offItemFirstBit(LV2_BOSS_KEY);
    dComIfGs_offItemFirstBit(BOMB_BAG_LV2);
    dComIfGs_offItemFirstBit(TOMATO_PUREE);
    dComIfGs_offItemFirstBit(TASTE);
    dComIfGs_offItemFirstBit(POU_FIRE1);
    dComIfGs_offItemFirstBit(POU_FIRE2);
    dComIfGs_offItemFirstBit(POU_FIRE3);
    dComIfGs_offItemFirstBit(POU_FIRE4);

    for (int i = 0; i < 24; i++) {
        dComIfGs_offItemFirstBit(i + M_BEETLE);
    }

    dComIfGs_offItemFirstBit(LIGHT_SWORD);
    dComIfGs_offItemFirstBit(SHIELD);
    dComIfGs_offItemFirstBit(ZORAS_JEWEL);

    for (int i = 0; i < 19; i++) {
        dComIfGs_offItemFirstBit(i);
    }

    dComIfGs_setCollectSmell(SMELL_PUMPKIN);

    if (!mDoCPd_c::isConnect(2)) {
        dComIfGs_offItemFirstBit(SMELL_POH);
    }
    ...
    dComIfGs_setBombMax(NORMAL_BOMB, 30);
    dComIfGs_setBombMax(WATER_BOMB, 15);
    dComIfGs_setBombMax(POKE_BOMB, 10);
...
inline void daNpcMsg_setEvtNum(int) {}

inline bool dMeter2Info_isNextStage(const char*, s16, s16, s16) {
    return false;
}

inline void dMeter2Info_onTempBit(int) {}
        ...
        if (dSm_read_comp_spel(pos, i_data) == 0) {
            ...
            loop_1:
                ...
                if (value < 0) {
                    ...
                    switch (value) {
                    case 0:
                        dMeter2Info_setSword(NO_ITEM, false);
                        break;
                    case 1:
                        dMeter2Info_setSword(SWORD, false);
                        break;
                ...
                if (value < 0) {
                    ...
                    switch (value) {
                    case 0:
                        dMeter2Info_setShield(NO_ITEM, false);
                        break;
                    case 1:
                ...
                if (value < 0) {
                    ...
                    switch (value) {
                    case 0:
                        dScnMenu_setItem(SLOT_0, NO_ITEM);
                        break;
                    case 1:
                        dScnMenu_setItem(SLOT_0, BOOMERANG);
                        break;
                    }
                }
                ...
                if (value < 0) {
                    ...
                    OS_REPORT("\nbow max[%d]", value);
                    if (value == 0) {
                        dScnMenu_setItem(SLOT_4, NO_ITEM);
                    } else if (value <= 10) {
                        dScnMenu_setItem(SLOT_4, ARROW_10);
                    } else if (value <= 20) {
                        dScnMenu_setItem(SLOT_4, ARROW_20);
                    } else {
                        dScnMenu_setItem(SLOT_4, ARROW_30);
                    }
                ...
                if (value < 0) {
                    ...
                    OS_REPORT("\narror-fire[%d]", value);
                    dScnMenu_setItem(SLOT_4, LIGHT_ARROW);
                }
                ...
                if (value < 0) {
                    ...
                    OS_REPORT("\nbomb max [%d]", value);
                    if (value == 0) {
                        dScnMenu_setItem(SLOT_15, NO_ITEM);
                        dComIfGs_setBombMax(value);
                    } else {
                        dScnMenu_setItem(SLOT_15, BOMB_BAG_LV1);
                        dComIfGs_setBombMax(value);
                    }
                }
                ...
                if (value < 0) {
                    ...
                    if (value <= 4) {
                        for (int i = 0; i < value; i++) {
                            dScnMenu_setItem((u8)(i + SLOT_11), EMPTY_BOTTLE);
                        }
                    }
                }
...
// NONMATCHING - regalloc, maybe equivalent?
s32 dScnMenu_Draw(dScnMenu_c* i_this) {
    ...
    // "(3B) Light:%s"
    JUTReport(290, 400, "(３Ｂ)ライト:%s", light_setting[g_env_light.field_0x1308 & 0xF]);
    // "(3X) Moon:%s"
...
s32 toGameScene(dScnMenu_c* i_this) {
    s32 rt;
    ...
    {
        g_env_light.mPondSeason = dComIfGp_getNextStageLayer() + 1;
    }
    ...
    if (strcmp(dComIfGp_getNextStageName(), "OPENING") == 0) {
        ...
        if (g_env_light.field_0x1308 != 0) {
            g_env_light.field_0x1308 = (u8)(g_env_light.field_0x1308 | 0xF0);
        }
...
void setEnvData(menu_data_class* i_data) {
    ...
    if (time != 99) {
        ...
        g_env_light.mDaytime = 15.0f * (f32)time;
    }
    ...
    for (int i = 0; i < 65; i++) {
                   ...
                   i_data->env_data[i + 5] >= '0' && i_data->env_data[i + 5] <= '9')
        {
            ...
            while (true) {
                ...
                int label_no = atoi(string);
                if (label_no >= 0 && label_no <= 999) {
                    dComIfGs_onTmpBit(dSv_event_tmp_flag_c::tempBitLabels[label_no]);
            ...
            while (true) {
                ...
                int label_no = atoi(string);
                if (label_no >= 0 && label_no <= 999) {
                    dComIfGs_offTmpBit(dSv_event_tmp_flag_c::tempBitLabels[label_no]);
            ...
            // "Iron Boots"
            dScnMenu_setItem(SLOT_3, HVY_BOOTS);
...
s32 dScnMenu_Execute(dScnMenu_c* i_this) {
    menu_info_class* menu_info = i_this->info;
        ...
        if (dScnMenu_c::m_autoCursolID > menu_info->num) {
            ...
            if (toGameScene(i_this)) {
                ...
                do {
                    ...
                    static u8 l_autoCancel[2][2] = {
                        {99, 99},
                        {0, 0},
                    };

                    u8* sp34 = l_autoCancel[0];
                    for (u32 i = 0; i < 1; i++) {
...
block_24:
    ...
    if (mDoCPd_c::getTrigB(PAD_3)) {
        g_env_light.field_0x1308++;
        if (g_env_light.field_0x1308 > 7) {
    ...
    g_env_light.mTimeSpeed = 0.0f;

    if (l_timepat >= 6) {
        f32 time = 15.0f * (f32)(l_timepat - 6);
        dComIfGs_setTime(time);
        g_env_light.mTimeSpeed = 1000.0f;
    } else {
        switch (l_timepat) {
        case 0:
        case 1:
        ...
        case 2:
            ...
            g_env_light.mDaytime = 90.0f;
            g_env_light.mTimeSpeed = 1000.0f;
            break;
        case 3:
            dComIfGs_setTime(195.0f);
            g_env_light.mDaytime = 195.0f;
            g_env_light.mTimeSpeed = 1001.0f;
            break;
        case 4:
            dComIfGs_setTime(255.0f);
            g_env_light.mDaytime = 255.0f;
            g_env_light.mTimeSpeed = 1002.0f;
            break;
        case 5:
            dComIfGs_setTime(315.0f);
            g_env_light.mDaytime = 315.0f;
            g_env_light.mTimeSpeed = 1003.0f;
            break;
        }
    }
...
s32 dScnMenu_IsDelete(dScnMenu_c* i_this) {
    return 1;
}

s32 dScnMenu_Delete(dScnMenu_c* i_this) {
...
// NONMATCHING - JUT_ASSERT macro seems wrong, otherwise equivalent
s32 phase_1(dScnMenu_c* i_this) {
        ...
        while (--index >= 0) {
            ...
            for (s8 i = 0; i < i_info->stage_data[index].field_0x41; i++) {
                if (strcmp(data->stage_name, dScnMenu_c::cursolStageName) == 0) {
                    ...
                    dScnMenu_c::m_error_flags &= ~ERR_MAP;
                    if (data->room_no == dScnMenu_c::cursolRoomNo) {
                        dScnMenu_c::m_error_flags &= ~ERR_ROOM;
                        if (data->layer == dScnMenu_c::cursolLayer) {
                            dScnMenu_c::m_error_flags &= ~ERR_LAYER;
                            if (data->point == dScnMenu_c::cursolPoint) {
                                dScnMenu_c::m_error_flags &= ~ERR_POINT;
                                return rt;
                            }
...
// NONMATCHING - vars using registers instead of stack, equivalent?
s32 phase_2(dScnMenu_c* i_this) {
    ...
    i_this->category_info =
        (menu_category_class*)((u32)i_this->command->getMemAddress() + category_offset);
    JUT_ASSERT(3282, i_this->category_info != 0);

    menu_category_class* category_info = i_this->category_info;
...
void dScnMenu_setPlayerDebugMode() {
    if (mDoCPd_c::isConnect(PAD_3) || !dScnMenu_setPlayerDebugModeDungeon()) {
        if (strcmp(dComIfGp_getNextStageName(), "D_MN05") == 0) {
            dScnMenu_setItem(SLOT_11, EMPTY_BOTTLE);
            dScnMenu_setItem(SLOT_12, RED_BOTTLE);
                   ...
                   strcmp(dComIfGp_getNextStageName(), "D_MN04A") == 0)
        {
            ...
            dComIfGs_onItemFirstBit(L2_KEY_PIECES1);
            dComIfGs_onItemFirstBit(L2_KEY_PIECES2);
            dComIfGs_onItemFirstBit(L2_KEY_PIECES3);
            dComIfGs_onItemFirstBit(LV2_BOSS_KEY);
            dComIfGs_onItemFirstBit(BOW);
            dComIfGs_onItemFirstBit(HOOKSHOT);
            dComIfGs_onItemFirstBit(BOOMERANG);
            dComIfGs_onItemFirstBit(RED_BOTTLE);
            dComIfGs_onItemFirstBit(OIL_BOTTLE);
            dComIfGs_onItemFirstBit(FAIRY);
            dComIfGs_onItemFirstBit(KANTERA);
            dComIfGs_onItemFirstBit(PACHINKO);
            dComIfGs_onItemFirstBit(HVY_BOOTS);

            dScnMenu_setItem(SLOT_4, BOW);
            dScnMenu_setItem(SLOT_9, HOOKSHOT);
            dScnMenu_setItem(SLOT_0, BOOMERANG);
            dScnMenu_setItem(SLOT_13, FAIRY);
            dScnMenu_setItem(SLOT_12, OIL_BOTTLE);
            dScnMenu_setItem(SLOT_11, RED_BOTTLE);
            dScnMenu_setItem(SLOT_1, KANTERA);
            dScnMenu_setItem(SLOT_23, PACHINKO);
            dScnMenu_setItem(SLOT_3, HVY_BOOTS);
...
void dScnMenu_setPlayerItemReset() {
    ...
    dComIfGs_onItemFirstBit(WEAR_CASUAL);
...
bool dScnMenu_setPlayerDebugModeDungeon() {
    if (strcmp(dComIfGp_getNextStageName(), "D_MN05") == 0) {
        ...
        dScnMenu_setItem(SLOT_1, KANTERA);
        dScnMenu_setItem(SLOT_23, PACHINKO);
        dScnMenu_setItem(SLOT_11, OIL_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
        dComIfGs_setSelectItemIndex(SELECT_ITEM_X, SLOT_11);
        dComIfGs_setSelectItemIndex(SELECT_ITEM_Y, SLOT_12);
               ...
               strcmp(dComIfGp_getNextStageName(), "D_MN04A") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_3, HVY_BOOTS);
        dScnMenu_setItem(SLOT_4, BOW);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_11, OIL_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
               ...
               strcmp(dComIfGp_getNextStageName(), "D_MN01B") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_4, BOW);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_16, WATER_BOMB);
        dScnMenu_setItem(SLOT_11, OIL_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
               ...
               strcmp(dComIfGp_getNextStageName(), "D_MN10B") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_16, WATER_BOMB);
        dScnMenu_setItem(SLOT_11, GREEN_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
        dComIfGs_setSelectItemIndex(SELECT_ITEM_X, SLOT_9);
               ...
               strcmp(dComIfGp_getNextStageName(), "D_MN11B") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_1, KANTERA);
        dScnMenu_setItem(SLOT_23, PACHINKO);
        dScnMenu_setItem(SLOT_2, SPINNER);
        dScnMenu_setItem(SLOT_3, HVY_BOOTS);
        dScnMenu_setItem(SLOT_4, BOW);
        dScnMenu_setItem(SLOT_9, HOOKSHOT);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
               ...
               strcmp(dComIfGp_getNextStageName(), "T_MN06") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_4, BOW);
        dScnMenu_setItem(SLOT_9, HOOKSHOT);
        dScnMenu_setItem(SLOT_6, IRONBALL);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_16, WATER_BOMB);
        dScnMenu_setItem(SLOT_11, FAIRY);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
               ...
               strcmp(dComIfGp_getNextStageName(), "T_MN07") == 0)
    {
        ...
        dScnMenu_setItem(SLOT_9, HOOKSHOT);
        dScnMenu_setItem(SLOT_6, IRONBALL);
        dScnMenu_setItem(SLOT_8, COPY_ROD);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_16, WATER_BOMB);
        dScnMenu_setItem(SLOT_11, GREEN_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
        ...
        dScnMenu_setItem(SLOT_6, IRONBALL);
        dScnMenu_setItem(SLOT_8, COPY_ROD);
        dScnMenu_setItem(SLOT_10, W_HOOKSHOT);
        dScnMenu_setItem(SLOT_15, NORMAL_BOMB);
        dScnMenu_setItem(SLOT_16, WATER_BOMB);
        dScnMenu_setItem(SLOT_11, BLUE_BOTTLE);
        dScnMenu_setItem(SLOT_12, RED_BOTTLE);
